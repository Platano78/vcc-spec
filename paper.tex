...
Karpathy's autoresearch loop (2026) establishes three primitives for autonomous iterative improvement: a \emph{scriptable asset} (the single file the agent modifies), a \emph{scalar metric} (a single number determining improvement), and a \emph{time-boxed cycle} (fixed duration per experiment). VCC maps directly to this pattern: the scriptable asset is the deliverable file under repair, the scalar metric is the VCC criterion pass/fail (binary, immutable), and the time-boxed cycle is one repair attempt per iteration.

Iacono's analysis of autoresearch identifies the \emph{frozen metric} as the critical architectural constraint: ``A system that can rewrite both the exam and the answers will always pass.'' VCC enforces this separation---the acceptance criteria are locked during execution, and the agent has no mechanism to modify them. This evaluation lock is structurally identical to autoresearch's requirement that the scoring function sit in a separate, immutable file.

The AgentForge implementation extends the autoresearch pattern with \emph{dogfooding}: after each repair attempt, the specific VCC criterion is re-evaluated via a callback (\texttt{VerifyAtomFn}). If the criterion still fails, the \emph{new} failure message is fed back as context for the next attempt, ensuring the model sees its own failed fix rather than blindly assuming success. \emph{Stagnation detection} halts the loop if no improvement (fewer failing criteria) is observed for $n$ consecutive outer iterations, preventing unbounded spinning without hard-capping iteration count.
