...
We propose \textbf{Verifiable Completion Contracts (VCC)}, a declarative specification format that defines the necessary and sufficient conditions for declaring an autonomous run complete. VCC is released as an open specification to encourage adoption across diverse autonomous systems. A reference implementation is available in AgentForge (npm: \texttt{forgeagent}) v3.0. The central design choice is a \emph{termination law}: a run may terminate successfully if and only if no \texttt{MUST} acceptance criteria are failing.
...
\subsection{The Autoresearch Pattern and Frozen Metrics}
Karpathy's autoresearch loop \cite{karpathy_autoresearch} establishes three primitives for autonomous iterative improvement: a \emph{scriptable asset} (the single file the agent modifies), a \emph{scalar metric} (a single number determining improvement), and a \emph{time-boxed cycle} (fixed duration per experiment). VCC maps directly to this pattern: the scriptable asset is the deliverable file under repair, the scalar metric is the VCC criterion pass/fail (binary, immutable), and the time-boxed cycle is one repair attempt per iteration.

Iacono's analysis of autoresearch \cite{karpathy_autoresearch} identifies the \emph{frozen metric} as the critical architectural constraint: ``A system that can rewrite both the exam and the answers will always pass.'' VCC enforces this separation---the acceptance criteria are locked during execution, and the agent has no mechanism to modify them. This evaluation lock is structurally identical to autoresearch's requirement that the scoring function sit in a separate, immutable file.

The AgentForge implementation combines the autoresearch pattern with the Ralph Loop \cite{ghuntley_ralph_loop}, a clean-context repair mechanism that wipes LLM context and re-feeds only the broken file and error. This extends the pattern with \emph{dogfooding}: after each repair attempt, the specific VCC criterion is re-evaluated via a callback (\texttt{VerifyAtomFn}). If the criterion still fails, the \emph{new} failure message is fed back as context for the next attempt, ensuring the model sees its own failed fix rather than blindly assuming success. \emph{Stagnation detection} halts the loop if no improvement (fewer failing criteria) is observed for $n$ consecutive outer iterations, preventing unbounded spinning without hard-capping iteration count.
...
\paragraph{Reference Implementation.} A production implementation of VCC is available in AgentForge (npm: \texttt{forgeagent}) v3.0. The implementation includes the reference JSON Schema, TypeScript adapter interfaces, and universal validators for structure, traceability, consistency, provenance, and AI-judged rubrics.

\subsection{Reference Schema and Adapter Interfaces}
A reference JSON Schema for VCC v1 was implemented to support machine validation and forward-compatible evolution (Schema ID: \texttt{https://agentforge.dev/schemas/vcc/v1/schema.json}).
...
\section{Empirical Evaluation: AgentForge Contract-Driven Convergence Regression}

This section reports a focused case study drawn from the AgentForge v3.0.0-alpha test regimen (2026-01-09), where runs were marked \texttt{COMPLETED} despite failing \texttt{MUST} acceptance criteria. The objective is to validate that VCC's semantics eliminate this failure mode by making acceptance satisfaction the sole success condition.

\subsection{System Under Test}
AgentForge is a production-grade autonomous workflow framework with a large codebase and extensive automated test coverage. The system implements iterative refinement, deliverable validation, and governance controls, but (prior to contract-driven convergence) allowed procedural completion to imply success.

\subsection{Regression Scenario}
In the reported tests, quality scores fell below configured thresholds, yet the run status was \texttt{COMPLETED}. Table~\ref{tab:agentforge_regression} summarizes the observed failure.
...
\bottomrule
\end{tabular}
\caption{AgentForge v3.0.0-alpha regression motivating contract-driven convergence: procedural completion was incorrectly treated as success.}
\label{tab:agentforge_regression}
\end{table}

\subsection{Observed Behavior}
Under VCC semantics (Definitions~1--3), real execution validates the termination law. Table~\ref{tab:e2e_results} summarizes E2E validation with a local LLM (agents-nemotron, 286.6s total execution, \$0.00 cost).

\begin{table}[t]
\centering
\caption{E2E validation results from real orchestration with contract-driven convergence enabled.}
\label{tab:e2e_results}
\begin{tabular}{lcc}
\toprule
Metric & Task 1 & Task 2 \\
\midrule
Termination Status & \texttt{SUCCESS} & \texttt{FAILURE} \\
Termination Reason & criteria\_satisfied & max\_passes \\
Final Quality Score & 73/100 & 54/100 \\
Peak Quality Score & 73/100 & 78/100 \\
Iterations Executed & 1 & 5 \\
Failing MUST Criteria & 0 & 1 \\
Duration & 55.3s & 231.1s \\
\bottomrule
\end{tabular}
\end{table}

Task~1 achieved \texttt{SUCCESS} on the first iteration because all \texttt{MUST} acceptance criteria were satisfied (quality score 73 exceeded threshold 60). Task~2 terminated as \texttt{FAILURE} after exhausting 5 iterations: despite reaching peak quality of 78/100 on iteration~2, subsequent stagnation and regression left the final quality at 54/100, below the threshold. The system correctly returned \texttt{FAILURE} with an explicit record of the failing criterion (``Quality score 54 below threshold'').

This confirms the termination law: $\mathrm{TerminateSuccess} \iff \mathrm{Satisfied}(\mathcal{C}_M)$. Notably, Task~2's peak quality of 78 did \emph{not} cause premature success---the system correctly required sustained MUST satisfaction, not momentary peaks.

\subsection{Implementation Refinements Discovered}
Real execution revealed a schema alignment issue: the VCC YAML specification uses \texttt{artifacts} (aligned with common CI/CD terminology) while the internal TypeScript interface expected \texttt{deliverables}. This was resolved by adding a \texttt{normalizeVCC()} adapter that maps external schema conventions to internal representations, preserving backward compatibility while allowing idiomatic YAML authoring. This discovery validates the importance of E2E testing over unit-test-only validation.

\subsection{Metrics and Future Benchmarking}
The E2E validation confirms correctness of VCC enforcement. For future benchmarking across diverse workloads, the following metrics are recommended:
...
\subsection{Production Benchmark: AgentForge v1.0.0-alpha.1 (March 2026)}
Table~\ref{tab:agentforge_benchmarks} reports results from 10 benchmark runs across 5 projects and 2 local LLMs, exercising the full six-gate pipeline including VCC-driven Ralph Loop repair with dogfooding and stagnation detection.

\begin{table}[t]
\centering
\caption{AgentForge production benchmarks (March 2026). OmniCoder-9B on RTX 3080 Ti 12GB, NousCoder-14B on RTX 5080 16GB. All runs self-contained (zero cloud dependencies).}
\label{tab:agentforge_benchmarks}
\begin{tabular}{lcc}
\toprule
Project & OmniCoder-9B & NousCoder-14B \\
\midrule
Zeeter (Twitter clone, JWT, SQLite) & PASS 96\%, 4.1 min & PASS, 17 min \\
RealWorld Conduit (19-endpoint API) & FAIL (36 TS errors) & PASS, 7.3 min \\
Flight Simulator (WebGL, single file) & FAIL (instruction) & PASS, 11.3 min \\
Browser OS (desktop simulator) & PASS, 1.8 min & PASS, 4.5 min \\
Bumper Cars (2D game, AI opponents) & PASS, 1.7 min & PASS 100\%, 17.8 min \\
\midrule
\textbf{Total} & \textbf{3/5 (60\%)} & \textbf{5/5 (100\%)} \\
\bottomrule
\end{tabular}
\end{table}

Key observations: (i)~VCC Repair Gate converged on every successful run (3 criteria resolved in 1 outer iteration, 3--7 seconds overhead). (ii)~The 9B model failures were correctly identified by VCC as model quality limitations, not pipeline failures---VCC attempted repair but the model could not produce correct multi-file TypeScript within its capability ceiling. (iii)~Compound learning persisted diagnosis entries across runs via JSONL, and prior failure patterns were injected into subsequent repair prompts. (iv)~Dogfooding via \texttt{VerifyAtomFn} callback prevented the inner loop from marking criteria resolved without re-evaluation, eliminating the blind-success failure mode observed in earlier iterations.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Validation Evidence}: Raw execution logs and generated deliverables are included in the supplementary materials, enabling independent verification of empirical claims.
\end{itemize}
The specification, reference implementation, and examples are available at \url{https://github.com/Platano78/vcc-spec}.

\paragraph{Versioning.} This paper documents VCC v1.0. Future versions may extend constraint expressiveness, add validator types, or support compositional contracts while preserving backward compatibility at the schema level.
